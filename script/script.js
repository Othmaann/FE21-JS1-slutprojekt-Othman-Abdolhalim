const Key = "cc128a90e8254895b95c7a8fb4344bf6";
...
userSubmit.addEventListener("click", function (e) {
  ...
  let urlCurrent = `https://api.weatherbit.io/v2.0/current?key=cc128a90e8254895b95c7a8fb4344bf6&lang=sv&units=m&city=${city}&country=${country}`;
  let urlForeCast = `https://api.weatherbit.io/v2.0/forecast/daily?city=${city}&lang=sv&country=${country}&key=${Key}`;
  ...
  if (userInput.value === "") {
    ...
    clearError();
    //liten sak med jQuery
    $(divBorder).on("mouseenter", function () {
      $("#results").css("backgroundColor", "purple");
      $("#results").css("color", "white");
    });

    $(divBorder).on("mouseleave", function () {
      $("#results").css("backgroundColor", "transparent");
      $("#results").css("color", "purple");

    });
    //första fetch för current api
    fetch(urlCurrent)
